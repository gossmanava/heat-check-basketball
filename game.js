...
let W=0,H=0,dpr=1,mode=30,state='menu',score=0,streak=0,bestStreak=0,made=0,shots=0,timeLeft=30,lastTime=0,timerCarry=0,sound=true,recordBroken=false;
...
function render(){court();if(state!=='menu')drawHoop();ripples.forEach(r=>{ctx.globalAlpha=r.a;ctx.strokeStyle='#ffbd19';ctx.lineWidth=4;ctx.beginPath();ctx.arc(r.x,r.y,r.r,0,Math.PI*2);ctx.stroke()});ctx.globalAlpha=1;if(state==='playing'||state==='paused'){drawBallCart();const readyBall=ball,wasDragging=dragging;dragging=false;projectiles.forEach(shot=>{ball=shot;drawBall()});ball=readyBall;dragging=wasDragging;drawBall()}particles.forEach(p=>{ctx.globalAlpha=Math.max(0,p.life);ctx.fillStyle=p.color;ctx.fillRect(p.x,p.y,5,5)});fireworks.forEach(p=>{ctx.globalAlpha=Math.max(0,p.life);ctx.strokeStyle=p.color;ctx.lineWidth=p.size;ctx.shadowColor=p.color;ctx.shadowBlur=8;ctx.beginPath();ctx.moveTo(p.px,p.py);ctx.lineTo(p.x,p.y);ctx.stroke()});ctx.shadowBlur=0;ctx.globalAlpha=1}
